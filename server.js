...
/* ---------- CLEAR (compat: /case/delete) ---------- */
app.post("/case/delete", (req, res) => {
  console.log("POST /case/delete called", { body: req.body });
  try {
    const { caseId } = req.body;
    let data = JSON.parse(fs.readFileSync(EVIDENCE_FILE));
    ...
    console.log("Deleted records", { caseId, removed: beforeCount - afterCount });
    return res.json({ success: true, message: "Case cleared successfully" });
  } catch (err) {
    console.error("Error in /case/delete", err);
